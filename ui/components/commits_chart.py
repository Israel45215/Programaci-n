...
def _week_column(week_label, commits_value, pr_value):
    bars = ft.Row(
        [
            _bar(commits_value, theme.ACCENT_COLOR),
            _bar(pr_value, getattr(theme, "PURPLE", "#8b5cf6")),
        ],
        spacing=5,
    ...
    return ft.Column(
        [bars, ft.Text(week_label, size=10.5, color=theme.TEXT_SECONDARY)],
        spacing=8,
...
def _y_axis_labels() -> ft.Column:
    ...
    return ft.Column(
        [ft.Text(f"{v}%", size=10, color=theme.TEXT_SECONDARY) for v in values],
        spacing=0,
...
def build_commits_chart() -> ft.Container:
    ...
    chart_body = ft.Row(
        [
            ...
            ft.Container(
                ...
                expand=True,
                padding=ft.padding.only(left=14) if hasattr(ft.padding, "only") else 14,
            ),
        ],
        spacing=8,
    )
    ...
    header = ft.Row(
        [
            ...
            _legend_dot(theme.ACCENT_COLOR, "Commits"),
            ft.Container(width=14),
            _legend_dot(getattr(theme, "PURPLE", "#8b5cf6"), "Pull requests"),
        ]
    )
